...
type PodFormModalProps = {
  mode: "create" | "edit";
  pod?: Pod;
  onClose: () => void;
  onSubmit: (values: PodFormValues) => Promise<void>;
};

/**
 * Modale de création/modification d'un pod (Bootstrap Modal + Bootstrap
 ...
 * ouverte).
 */
export default function PodFormModal({ mode, pod, onClose, onSubmit }: PodFormModalProps) {
  const titleId = "pod-form-modal-title";
  ...
  const formId = "pod-form";

  return (
    <ModalShell
      titleId={titleId}
      title={mode === "create" ? "Créer un pod" : "Modifier le pod"}
      onClose={onClose}
      footer={
        <>
